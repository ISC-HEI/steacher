...
class TestCase(BaseModel):
    ...
    description: str = Field(description="The description of the test case, in english. Not mandatory, but recommended to give context to the test case.")
    test_code: str = Field(description="The test code. It should use the student's answer and either make an `assert` statement or `print` something to the console. For example: `assert student_function(1, 2) == 3` or `print(student_function('test'))`.")
    expected_output: str = Field(description="The expected output of the test code in the console. This should be an exact match, including newlines (though the string will be trimmed of whitespace). For a test that prints the number 15, this should be '15\\n'.")


class UnitTests(BaseModel):
    """Pydantic model for unit tests structure."""
    setup_code: str = Field(default="", description="Setup code to run before the tests. Optional. Typically used to set up the environment for the tests, like importing modules or defining helper functions.")
...
class AnswerData(BaseModel):
    ...
    For example, for a SQL exercice, it would be the result of a query. For a simple python exercice that prints to the console, it would be the content of the console output. For an open question, it would be the expected answer.''')
...
def get_pydantic_schema_as_string() -> str:
    """
    Get the Pydantic schema as a string to be used in the prompt.
    """
